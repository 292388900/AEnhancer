...
public class Aggregation implements Iterable {
     ...
     */
    public Aggregation(Class<?> target) {
        this.target = target;
        datas = new LinkedList<Object>();
        assertAggregationType();
    }

    public Aggregation(Class<?> target, Object input) {
        this(target);
        addAll(input);
    }
     ...
     * @throws IllegalParamException
     */
    private void addAll(Object input) {
        if (null == input) {
    ...
    public void add(Aggregation agg) {
        if (agg.target != target) {
            throw new IllegalParamException("the aggr param's type is error,one is : " + target + " another is "
                    + agg.target);
        }
     ...
     * @throws IllegalParamException
     */
    public Iterator getSeqIteratorFromInstance(Object instance) {
        if (null == instance) {
     ...
     * @throws IllegalParamException
     */
    public List<Aggregation> split(int batchSize) {
